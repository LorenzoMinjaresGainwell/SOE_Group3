...
SOURCES = [('wa_hca_provider_alerts', 'https://www.hca.wa.gov/billers-providers-partners/provider-alerts-2026', 'provider_bulletin', ['provider alert', 'apple health alert', 'billing guide', 'prior authorization', 'effective']), ('wa_hca_spas', 'https://www.hca.wa.gov/about-hca/programs-and-initiatives/apple-health-medicaid/2026-state-plan-amendments', 'spa_notice', ['state plan amendment', 'spa ', 'public notice']), ('wa_hca_rht', 'https://www.hca.wa.gov/about-hca/programs-and-initiatives/rural-health-transformation-program', 'rht_notice', ['rural health transformation', 'rht', 'funding', 'participate'])]
CONTEXT_TERMS = ['apple health', 'medicaid', 'provider', 'state plan amendment', 'waiver', 'rural health transformation']
...
def fetch_updates(*, keywords: list[str], max_records: int, progress: Callable[[str], None] | None = None) -> list[dict[str, str]]:
    ...
    for source, url, default_type, terms in SOURCES:
        try:
            ...
            emit(progress, f"WA {source} unavailable: {exc}")
            continue
        accepted = [to_record(row, source, url, default_type, keywords) for row in rows if keep_row(row, keywords)]
        records.extend(accepted)
        emit(progress, f"WA {source}: scanned {len(rows)} candidate links, normalized {len(accepted)}")
    output = sorted(unique_records(records), key=sort_key, reverse=True)
    emit(progress, f"WA: normalized {len(output)} official policy updates")
...
def to_record(row: dict[str, str], source: str, source_url: str, default_type: str, keywords: list[str]) -> dict[str, str]:
    ...
    return state_update_record(
        state='WA', source=source, source_record_id=source_id_from_url(row["url"]) or row["title"],
