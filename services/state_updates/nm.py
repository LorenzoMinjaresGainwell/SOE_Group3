...
SOURCES = [('nm_hca_provider_communications', 'https://www.hca.nm.gov/communications-to-providers/', 'provider_bulletin', ['provider communication', 'mad ', 'billing', 'supplement', 'provider update']), ('nm_hca_public_comments', 'https://www.hca.nm.gov/public-information-and-communications/opportunity-for-public-comment/public-notices-proposed-waiver-changes-and-opportunities-to-comment/comment-period-open/', 'public_comment_notice', ['comment', 'waiver', 'public notice', 'medicaid']), ('nm_hca_state_plan', 'https://www.hca.nm.gov/new-mexico-medicaid-state-plan/', 'spa_notice', ['state plan', 'spa', 'amendment']), ('nm_hca_rht', 'https://www.hca.nm.gov/rht/', 'rht_notice', ['rural health', 'rht', 'fund', 'application'])]
CONTEXT_TERMS = ['new mexico medicaid', 'provider', 'state plan', 'waiver', 'rural health']
...
def fetch_updates(*, keywords: list[str], max_records: int, progress: Callable[[str], None] | None = None) -> list[dict[str, str]]:
    ...
    for source, url, default_type, terms in SOURCES:
        try:
            ...
            emit(progress, f"NM {source} unavailable: {exc}")
            continue
        accepted = [to_record(row, source, url, default_type, keywords) for row in rows if keep_row(row, keywords)]
        records.extend(accepted)
        emit(progress, f"NM {source}: scanned {len(rows)} candidate links, normalized {len(accepted)}")
    output = sorted(unique_records(records), key=sort_key, reverse=True)
    emit(progress, f"NM: normalized {len(output)} official policy updates")
...
def to_record(row: dict[str, str], source: str, source_url: str, default_type: str, keywords: list[str]) -> dict[str, str]:
    ...
    return state_update_record(
        state='NM', source=source, source_record_id=source_id_from_url(row["url"]) or row["title"],
