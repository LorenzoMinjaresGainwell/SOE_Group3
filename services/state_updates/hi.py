...
SOURCES = [('hi_medquest_provider_bulletins', 'https://medquest.hawaii.gov/en/plans-providers/fee-for-service/provider-bulletins.html', 'provider_bulletin', ['provider bulletin']), ('hi_medquest_provider_memos', 'https://medquest.hawaii.gov/en/plans-providers/provider-memo.html', 'provider_bulletin', ['qi-26', 'ffs 26', 'memo qi-26']), ('hi_medquest_waiver_news', 'https://medquest.hawaii.gov/en/about/recent-news/2026-news-articles.html', 'medicaid_notice', ['waiver', 'public notice', 'rural health']), ('hi_medquest_state_plan', 'https://medquest.hawaii.gov/en/about/state-plan-1115.html', 'waiver_notice', ['state plan amendment', '1115', 'waiver'])]
CONTEXT_TERMS = ['med-quest', 'hawaii medicaid', 'provider', 'state plan', '1115', 'waiver']
...
def fetch_updates(*, keywords: list[str], max_records: int, progress: Callable[[str], None] | None = None) -> list[dict[str, str]]:
    ...
    for source, url, default_type, terms in SOURCES:
        try:
            ...
            emit(progress, f"HI {source} unavailable: {exc}")
            continue
        accepted = [to_record(row, source, url, default_type, keywords) for row in rows if keep_row(row, keywords)]
        records.extend(accepted)
        emit(progress, f"HI {source}: scanned {len(rows)} candidate links, normalized {len(accepted)}")
    output = sorted(unique_records(records), key=sort_key, reverse=True)
    emit(progress, f"HI: normalized {len(output)} official policy updates")
...
def to_record(row: dict[str, str], source: str, source_url: str, default_type: str, keywords: list[str]) -> dict[str, str]:
    ...
    return state_update_record(
        state='HI', source=source, source_record_id=source_id_from_url(row["url"]) or row["title"],
